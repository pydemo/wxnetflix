...
from ui_layer.module.Searcheable_Tree2 import Searcheable_Tree2
...
#---------------------------------------------------------------------------
class SearcheableObjectFieldsCtrl(wx.Panel, Base, EditMenu):
    def __init__(self,  **kwargs):
        ...
        self.st = st = Searcheable_Tree2(tree_panel)
        self.tree=st.tree
        if 0:
    ...
    def show_data(self):
        ...
        for k in [k for k in self.pdef if k not in ['ResponseMetadata']]:
            ...
            for d in self.pdef[k]:
                ...
                v[f"{d['id']} [{d['name']}]"]=fld=[]
                for f in d['fields']:
                    fld.append(f"{f['key']}:{f.get('stringValue','None')}")
            treeList.append((k,v))
...
#---------------------------------------------------------------------------
def runTest(**kwargs):
    win = SearcheableObjectFieldsCtrl(**kwargs)
