...
def sum_col(df, col, new_col):
        ...
        return df        
class PlotPanel_year(wx.Panel, Base):
    def __init__(self, parent=None):
        super(PlotPanel_year, self).__init__(parent)
    ...
    def Plot (self):
        ...
        if 1:
            ...
            sns.histplot(data=self.data, x="release_year", shrink=.8, ax=self.axes, binwidth=3, kde=True).set(title='Release year/count')
        if 0:
            for ax in self.axx:
                ax.cla()        
